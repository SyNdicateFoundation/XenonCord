...
public class ChannelWrapper {
    ...
    @Getter private volatile boolean closed, closing;
    ...
    public void updateComposite() {
        final CipherEncoder cipherEncoder = ch.pipeline().get(CipherEncoder.class);
        final PacketCompressor packetCompressor = ch.pipeline().get(PacketCompressor.class);
        final Varint21LengthFieldPrepender prepender = ch.pipeline().get(Varint21LengthFieldPrepender.class);
        final boolean compressorCompose = cipherEncoder == null || cipherEncoder.getCipher().allowComposite();
        final boolean prependerCompose = compressorCompose && (packetCompressor == null || packetCompressor.getZlib().allowComposite());

        if (prepender != null) {
            ProxyServer.getInstance().getLogger().log(Level.FINE, "set prepender compose to {0} for {1}", new Object[]{prependerCompose, ch});
            prepender.setCompose(prependerCompose);
        }
        if (packetCompressor != null) {
            ProxyServer.getInstance().getLogger().log(Level.FINE, "set packetCompressor compose to {0} for {1}", new Object[]{compressorCompose, ch});
